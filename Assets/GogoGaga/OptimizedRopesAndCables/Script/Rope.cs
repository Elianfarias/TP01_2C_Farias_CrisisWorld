...
namespace GogoGaga.OptimizedRopesAndCables
{
    ...
    public class Rope : MonoBehaviour
    {
        ...
        {
            if (other.TryGetComponent<FsmNPCManager>(out FsmNPCManager npc))
            {
                if (npc.isCivil)
                {
                    ScoreManager.Instance.AddCivilSavedScore();
        ...
        private void UpdateCapsuleCollider()
        {
            ...
            Vector3 worldCenter = (startPoint + endPoint) / 2f;
        ...
        public void SetActiveRope(bool active)
        {
            gameObject.SetActive(active);
        }

        public bool IsActiveRope()
        {
        ...
        private void SetSplinePoint()
        {
            ...
            if (midPoint != null)
            {
                midPoint = GetRationalBezierPoint(startPoint, mid, endPoint, midPointPosition, StartPointWeight, midPointWeight, EndPointWeight);
            }

            for (int i = 0; i < linePoints; i++)
            {
                Vector3 p = GetRationalBezierPoint(startPoint, mid, endPoint, i / (float)linePoints, StartPointWeight, midPointWeight, EndPointWeight);
                lineRenderer.SetPosition(i, p);
            }

            lineRenderer.SetPosition(linePoints, endPoint);
        }
        ...
        public Vector3 GetPointAt(float t)
        {
            ...
            return GetRationalBezierPoint(startPoint, currentValue, endPoint, t, StartPointWeight, midPointWeight, EndPointWeight);
        }
